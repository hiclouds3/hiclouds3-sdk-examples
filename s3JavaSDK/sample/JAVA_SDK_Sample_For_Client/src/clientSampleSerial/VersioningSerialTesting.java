...
public class VersioningSerialTesting{
    ...
    {
    	AmazonS3 s3 = new AmazonS3Client(new PropertiesCredentials(VersioningSerialTesting.class.getResourceAsStream("AwsCredentials.properties")));
    ...
    private static void basicDisableBucketVersioning(String bucketName) throws IOException
    {
		...
		try
		{
			//設定Bucket Versioning功能為Suspended
	...
	private static void pGetBucketObjectVersions(String bucketName) throws IOException
	{
		...
		try
		{
			...
			//列出Bucket中所有版本的Object，搭配Max Key參數過濾結果
			...
			//列出Bucket中所有版本的Object，搭配Prefix參數過濾結果
			...
			//列出Bucket中所有版本的Object，搭配Key Marker參數過濾結果
			...
			//列出Bucket中所有版本的Object，搭配Delimeter & Prefix參數過濾結果
			...
			//列出Bucket中所有版本的Object，搭配Version ID Marker & Marker參數過濾結果
			System.out.println("get bucket versions Version-ID-MArker & key-marker");
			result = s3.listVersions(new ListVersionsRequest().withBucketName(bucketName).withKeyMarker("photos/2006/January/sample.jpg").withVersionIdMarker(vid));
			for(S3VersionSummary s : result.getVersionSummaries())
			{
				System.out.println(s.getBucketName());
				System.out.println(s.getKey());
