...
{
    /// <summary>
    ///    Response Unmarshaller for DeleteRankExpression operation
    /// </summary>
    internal class DeleteRankExpressionResponseUnmarshaller : XmlResponseUnmarshaller
    {

        public override AmazonWebServiceResponse Unmarshall(XmlUnmarshallerContext context) 
        {
            DeleteRankExpressionResponse response = new DeleteRankExpressionResponse();
            
            while (context.Read())
            {
                if (context.IsStartElement)                
                {
                    if(context.TestExpression("DeleteRankExpressionResult", 2))
                    {
                        response.DeleteRankExpressionResult = DeleteRankExpressionResultUnmarshaller.GetInstance().Unmarshall(context);
                        continue;
                    }
        ...
        private static DeleteRankExpressionResponseUnmarshaller instance;

        public static DeleteRankExpressionResponseUnmarshaller GetInstance()
        {
            if (instance == null) 
            {
               instance = new DeleteRankExpressionResponseUnmarshaller();
            }
            return instance;
        }
