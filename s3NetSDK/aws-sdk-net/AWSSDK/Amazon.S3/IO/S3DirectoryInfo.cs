...
namespace Amazon.S3.IO
{
    ...
    public sealed class S3DirectoryInfo : IS3FileSystemInfo
    {
        ...
        public S3DirectoryInfo(AmazonS3 s3Client, string bucket)
        ...
        /// <exception cref="T:System.ArgumentNullException"></exception>
        public S3DirectoryInfo(AmazonS3 s3Client, string bucket, string key)
        {
            if (s3Client == null)
            {
            ...
            if (!String.IsNullOrEmpty(bucket) && !String.IsNullOrEmpty(key) && !this.key.EndsWith("\\"))
            {
                this.key = string.Format("{0}{1}", this.key, "\\");
            }
        ...
        internal bool ExistsWithBucketCheck(out bool bucketExists)
        {
            ...
            catch (AmazonS3Exception e)
            {
                ...
                else if (string.Equals(e.ErrorCode, "NoSuchKey"))
                {
                    return false;
                }
                throw;
        ...
        public string FullName
        {
            get
            {
                return String.Format("{0}:\\{1}", bucket, key);
            }
        }
            ...
            get
            {
                ...
                if (Exists)
                {
                    if (String.IsNullOrEmpty(this.bucket))
                    {
                        ...
                        foreach (S3Bucket s3Bucket in s3Client.ListBuckets(new ListBucketsRequest().WithBeforeRequestHandler(S3Helper.FileIORequestEventHandler) as ListBucketsRequest).Buckets)
                        {
            ...
            if (recursive)
            {
                ...
                do
                {
                    ...
                    foreach (S3Object s3o in listResponse.S3Objects.OrderBy(x => x.Key))
                    {
                        ...
                        if (deleteRequest.Keys.Count == Util.S3Constants.MULTIPLE_OBJECT_DELETE_LIMIT)
                        {
                            s3Client.DeleteObjects(deleteRequest);
                            deleteRequest.Keys.Clear();
                        }
        ...
        public IEnumerable<S3DirectoryInfo> EnumerateDirectories(string searchPattern, SearchOption searchOption)
        {
            ...
            if (String.IsNullOrEmpty(bucket))
            {
                folders = s3Client.ListBuckets(new ListBucketsRequest().WithBeforeRequestHandler(S3Helper.FileIORequestEventHandler) as ListBucketsRequest).Buckets
            ...
            else
            {
                folders = new EnumerableConverter<string, S3DirectoryInfo>
                    ((IEnumerable<string>)(PaginatedResourceFactory.Create(new PaginatedResourceInfo()
                            ...
                            .WithTokenRequestPropertyPath("Marker")
                            .WithTokenResponsePropertyPath("NextMarker"))),
                        dir => new S3DirectoryInfo(s3Client,bucket,S3Helper.DecodeKey(dir)));
            }
            ...
            folders = folders.Where(s3dirInfo => Regex.IsMatch(s3dirInfo.Name.ToLower(), regEx));
            return folders;
        }
        ...
        public IEnumerable<S3FileInfo> EnumerateFiles(string searchPattern, SearchOption searchOption)
        {
            ...
            else
            {
                files = new EnumerableConverter<S3Object, S3FileInfo>
                    (((IEnumerable<S3Object>)(PaginatedResourceFactory.Create(new PaginatedResourceInfo()
                            ...
                            .WithTokenRequestPropertyPath("Marker")
                            .WithTokenResponsePropertyPath("NextMarker"))))
                        .Where(s3Object => !String.Equals(S3Helper.DecodeKey(s3Object.Key),key)),
            ...
            //filter based on search pattern
            var regEx = WildcardToRegex(searchPattern);
            files = files.Where(s3fileInfo => Regex.IsMatch(s3fileInfo.Name.ToLower(), regEx));
            return files;
        }
        ...
        public S3DirectoryInfo GetDirectory(string directory)
        {
            ...
            {
                ret = new S3DirectoryInfo(s3Client, bucket, String.Format("{0}{1}", key, directory));
            }
            return ret;
        }
        ...
        public S3FileInfo GetFile(string filename)
        {
            ...
            {
                ret = new S3FileInfo(s3Client, bucket, String.Format("{0}{1}", key, filename));
            }
            return ret;
        }
        ...
        public DirectoryInfo CopyToLocal(string path, DateTime changesSince)
        {
            ...
            if (LastWriteTime > changesSince)
            {
                foreach (S3DirectoryInfo dir in EnumerateDirectories())
                {
                    DirectoryInfo newLoc = Directory.CreateDirectory(String.Format("{0}\\{1}", path, dir.Name));
                ...
                foreach (S3FileInfo file in EnumerateFiles())
                {
                    if (file.LastWriteTime > changesSince)
                    {
                        file.CopyToLocal(String.Format("{0}\\{1}", path, file.Name));
                    }
                }
            }
        ...
        public DirectoryInfo MoveToLocal(string path)
        {
            ...
            foreach (S3FileInfo file in EnumerateFiles())
            {
                file.MoveToLocal(String.Format("{0}\\{1}", path, file.Name));
            }
        ...
        {
            return "^" + Regex.Escape(pattern.ToLower()).
            Replace("\\*", ".*").
            Replace("\\?", ".") + "$";
        }
